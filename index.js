...
fetch('din-api-endpoint', {
    method: 'GET',
    headers: {
      'Authorization': 'Bearer din-autentiserings-token' // Om autentisering krävs
    }
  })
    .then(response => {
      if (response.ok) {
        return response.json(); // Parsa och använd svaret här
      } else {
        throw new Error('Något gick fel');
      }
    })
    .then(data => {
      // Hantera data från GET-förfrågan här
    })
    .catch(error => {
    ...
    const dataToCreate = { key1: 'värde1', key2: 'värde2' };

fetch('din-api-endpoint', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer din-autentiserings-token', //Ofta har webbservar inloggning
    'Content-Type': 'application/json' // Ange rätt content-type för POST
  },
  body: JSON.stringify(dataToCreate) // Konvertera data till JSON och skicka som body (kroppen)
})
  .then(response => {
    if (response.ok) {
      return response.json();
  ...
  .then(data => {
    // Hantera svaret från POST-förfrågan här
  })
  .catch(error => {
    console.error('Fel:', error);
  });
...
const resourceId = 123; // Till exempel ID för den resurs du vill uppdatera
const updatedData = { key1: 'nytt-värde1', key2: 'nytt-värde2' };

//Glöm ej id
fetch(`din-api-endpoint/${resourceId}`, {
  method: 'PUT',
  headers: {
    'Authorization': 'Bearer din-autentiserings-token',
  ...
  .then(data => {
    // Hantera svaret från PUT-förfrågan här
  })
  .catch(error => {
    console.error('Fel:', error);
  });
  ...
  const resourceId = 123; // Resursens ID
